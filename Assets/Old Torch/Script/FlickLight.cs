...
public class FlickLight : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _startPosLight = lig.transform.localPosition;
    ...
    void MoveShadowLight()
    {
        if (moveShadow)
        {
            lig.transform.localPosition = _startPosLight + (Random.insideUnitSphere * speedMoveShadow / speedSmoothShadow);
        }
        else
        {
            moveShadow = false;
            lig.transform.localPosition = _startPosLight;
        }
    }
}
